...
class Microphone extends AudioWorkletProcessor {
  constructor() { super(); this.sum=0; this.count=0; this.phase=0; this.frame=new Int16Array(1600); this.index=0; }
  process(inputs) {
    ...
    if (input) for (const sample of input) {
      ...
      if (this.phase>=sampleRate) {
        ...
        if(this.index===this.frame.length){this.port.postMessage(this.frame.buffer,[this.frame.buffer]);this.frame=new Int16Array(1600);this.index=0;}
      }
    }
    return true;
  }
